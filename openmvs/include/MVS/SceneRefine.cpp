...
double MeshRefine::ScoreMesh(double* gradients)
{
	...
	//通过mesh投影到reference图像上的faces，投影到ref和src像素上
	scorePhoto = 0;
	...
	const Image32F& imageB, Image32F& imageA, BitMatrix& mask)
{
	...
	for (int j=0; j<depthMapA.rows; ++j) {
		for (int i=0; i<depthMapA.cols; ++i) {
			...
			if (!IsDepthSimilar(depthMapB, pt, ptC.z))//如果投影到B的深度与当前深度不相似有可能是遮挡区
				continue;
...
#endif // MESHOPT_CERES
//mesh优化，主要是采用光度一致性，代码用两种方式：1.调用ceres库，2.直接实现作者手写的最小化优化方法，两种效果和性能类似
//主要参考文献：high accuracy and visibility-consistent dense multi view stereo
//nresolutionlevel  用来refine的图片分辨率
//nminresolution 用来refine的图片最小分辨率
//nmaxviews 最大view个数
//fdecimatemesh  下采样率
//ncloseholes 用来补洞的阈值，洞大于该值才会补
//nensureedgesize  是否保持边界
//nmaxfacearea  refine过程中网格三角面的面积最大值（单位是像素）如果想让最终网格稠密可以将此减小
//nscales  控制用几个图像尺度去优化mesh 默认3
//fscalestep  尺度步长 默认0.5
//nreducememory  是否减少内存占用 默认1
//nalternatepair  可选择的图像对
//fregularityweight  权重系数
//fthplanarvertax  平面顶点阈值
//fgradientstep  梯度下降的步长
			...
			for (int iter=0; iter<iters; ++iter) {
				...
				if (bAdaptMesh) {
					...
					FOREACH(v, refine.vertices) {
						Vertex& vert = refine.vertices[v];
						const Point3d grad(gradients.row(v));
						//梯度下降，调整量=梯度*下降的步长（记得看一下最小二乘法的原理）
						vert -= Cast<Vertex::Type>(grad*gstep);
						gv += norm(grad);
					}
				}
