...
void DepthEstimator::ProcessPixel(IDX idx)
{
	...
	const Normal viewDir(Cast<float>(reinterpret_cast<const Point3&>(X0)));//viewdir表示坐标原点到邻域帧中心点向量
	...
	//随机分配：在当前depth值下，上下加随机值来找最优值来进一步优化
	...
	//OPTDENSE::fRandomDepthRatio：深度范围比率当前估计的随即平面分配
...
Depth DepthEstimator::InterpolatePixel(const ImageRef& nx, Depth depth, const Normal& normal) const
{
	...
	//利用nx=d平面方程计算depth，x=depthnew*x0，n表示法向量，x表示平面上的空间点
	{
		#if 0
